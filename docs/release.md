...
`granite.config.ts`의 `brand.displayName`과 **같은 값이어야 한다.**
지금은 `'썸타임레프트'`라 어긋나 있다 — 둘 다 사용자가 보는 이름이다.
`자투리 시간`으로 통일하면 설정도 한 줄 바꾼다:

```ts
brand: { displayName: '자투리 시간', ... }
```

토스 앱 목록에서 "썸타임레프트"는 읽어도 무슨 앱인지 알 수 없고,
"자투리 시간"은 이름만으로 무슨 시간을 다루는 앱인지 바로 읽힌다.
영어 이름과 저장소 이름은 `some-time-left` 그대로 둔다.
...
### 상세 설명 — "무엇을 보고, 어떤 버튼을 누르고, 무엇을 경험하는지"

```
약속 시각보다 일찍 도착해서 애매하게 남는 시간을, 걷기로 채워주는 앱이에요.

첫 화면에서 약속 장소와 약속 시각을 입력해요. 장소는 검색해서 고르고,
시각은 선택기로 정해요. 목적지까지 20분이면 닿는데 30분이 남았다면,
남는 10분을 걸을 수 있는 길을 만들어요. 시간이 빠듯하면 "지금 나서는 게
최선이에요"라고 알려주고 길을 만들지 않아요.

다음 화면에서 지금 기분을 하나 골라요. '설레요', '긴장돼요',
'생각이 많아요', '지쳤어요', '그냥 그래요', '햇볕이 싫어요' 여섯 개예요.
경사나 그늘, 혼잡도 같은 조건은 직접 고르지 않아요. 기분만 고르면
그에 맞는 길을 앱이 정해요.

경로 화면에서는 추천된 길 하나와, 왜 이 길인지 한 줄을 봐요.
"이 시간엔 그늘이 이어지는 길이에요", "신호등이 적어서 생각이 잘 안
끊겨요" 같은 문장이에요. '이 길로 갈게요'를 누르면 걷기가 시작돼요.
마음에 들지 않으면 '곧장 갈게요'로 최단 경로를 택할 수 있어요.

걷는 동안에는 남은 거리와 남은 시간, 그리고 페이스 안내를 봐요.
너무 빠르면 "조금 천천히 걸어도 돼요"라고 알려줘요. 목적지에 닿으면
도착 화면이 열려요.

도착 화면에서는 "걸으면서 무슨 생각 했어요?"라는 질문에 한 줄을 남길 수
있어요. '남기고 닫기'와 '그냥 닫기' 중에 고르면 되고, 남기지 않아도 돼요.

'지나온 길'에서는 지금까지 걸은 길의 모양이 격자로 쌓여요. 걸은 거리와
남긴 기록이 함께 보여요.

위치 권한을 사용해요. 지금 위치에서 목적지까지 길을 만들고, 걷는 동안
남은 거리를 계산하는 데에 써요. 길을 받기 위해 TMAP 보행자 경로 API에
좌표를 보내고, 그 밖에 위치를 따로 저장하거나 다른 곳으로 보내지 않아요.
걸은 기록은 기기 안에만 남아요.
```

마지막 문단은 코드와 맞춰 쓴 것이다. 좌표가 나가는 곳은
`src/data/tmap/`의 경로·검색 요청뿐이고, 혼잡도 프록시에는 좌표가 아니라
장소 이름만 간다. `src/data/records.ts`가 저장하는 기록은 앱인토스
Storage — 기기 안에만 남는다.
