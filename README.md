...
약속 장소 근처에 일찍 와 버려 애매하게 남은 시간을, 약속 3분 전에 딱 닿는 산책으로 바꿔 주는 앱인토스 미니앱.
(저장소와 `appName`은 `some-time-left`, 영어 이름은 `Some Time Left`.)

가장 빠른 길이 아니라, **약속 시각 앞에 딱 닿는 길**을 찾는다. 코드는 5분 전을 겨누고
화면은 3분 전을 약속한다 — 겨눈 값이 아니라 지킬 수 있는 값을 말한다.
...
| 목적지 혼잡도 (서울 인구데이터) | 연동 완료 — quiet과 같은 프록시를 쓴다 (앱 키·월 한도 없음) |
...
src/
  domain/            순수 로직. React Native에 의존하지 않아 CI에서 그대로 테스트된다.
    time.ts          "약속보다 먼저 도착" → 시간 예산. 이 앱의 출발점.
...
**몇 분 전인지는 설정값이 아니다.** `ARRIVE_EARLY_SEC` 상수로 고정하고 UI에서 바꾸게
하지 않는다. 임의적이고 구체적인 숫자 하나가 이 제품의 정체성이다.
바꾸는 건 사용자가 아니라 날씨뿐이다 — 비·눈이 오는 날은 7분(`WET_ARRIVE_EARLY_SEC`).
갈림은 `arriveEarlySecFor` 한 군데가 하고, 계획(`WalkPlan`)이 그날의 값을 들고 다닌다.
화면은 겨눈 값(5분)이 아니라 지킬 수 있는 값(바닥 3분, `promisedMinutes`)을 말하고,
내리는 날은 숫자 대신 "조금 더 일찍"이라고 말한다. 문구는 계획에서 뽑아 쓴다 —
상수를 손으로 적어 두면 비 오는 날 화면만 맑은 날 말을 한다.

처음엔 3분이었다. 실제로 걸어 보니 너무 얇았다 — 신호 하나, 횡단보도 한 번,
도로망 추정 오차 몇십 초면 그대로 약속 시각을 넘긴다. 5분이면 그 정도가 흡수되고,
넘치더라도 여전히 약속 전에 닿는다. **먼저 도착하는 앱이 늦는 것보다 나쁜 실패는 없다.**

그리고 목표보다 오래 걸리는 경로는 **한 뼘도 봐주지 않는다**(`arrivesOnTime`).
여유를 주는 자리는 후보를 고르는 문턱이 아니라 5분이라는 숫자 자체다.

**추천 이유 한 줄이 없으면 자동 추천은 실패한다.** 자동 추천의 신뢰는 정확도가 아니라
설명에서 나온다. `copy.ts`의 `routeReason()`이 그 역할을 하고, 화면의 주인공은 지도가
아니라 이 문장이다.

지도는 깔되(`RouteMap`) 옅은 회색뿐이라 그 문장에서 눈을 뺏지 않는다.
색을 가진 건 그 위의 경로 하나다. 타일이 안 와도 경로는 그대로 그려지므로
지도가 없다고 화면이 죽지 않는다.

지도는 두 판이 있고 `config.mapTiles.kind`가 고른다 — 래스터(CARTO 타일을 직접
깔고 SVG로 그린다, 가볍다)와 벡터(웹뷰 안 MapLibre + OpenFreeMap, 조건이 깨끗하다).
자세한 것은 [`docs/apps-in-toss-gotchas.md`](docs/apps-in-toss-gotchas.md)에.
...
### 목적지 혼잡도 (서울 실시간 인구데이터)

**약속 장소가 서울 주요 장소 근처일 때** 도착 화면에 한 줄 얹는다.

> "성수카페거리 일대, 지금 붐벼요." (남은 시간은 바로 위 큰 숫자가 말한다)

처음엔 TMAP Puzzle(대형 쇼핑시설 200여 곳)을 썼는데 두 가지 이유로 옮겼다.

1. **Puzzle은 앱 키의 월 사용량을 먹는다.** 실제로 한 달의 80%를 썼다.
   서울 인구데이터는 `quiet` 점수가 이미 쓰는 우리 프록시를 그대로 타고,
   프록시가 5분씩 캐시하므로 업스트림 호출도 뭉쳐진다. 앱에는 키가 없다.
2. **걷는 약속 장소와는 서울 주요 장소 쪽이 더 겹친다.** 쇼핑몰 목록에는
   "성수카페거리"도 "연남동"도 없다.

목적지 좌표에서 반경 안의 가장 가까운 장소를 찾고, 밖이면 조용히 생략한다 —
김포공항에서 "강남역이 붐벼요"라고 지어내지 않는다.
