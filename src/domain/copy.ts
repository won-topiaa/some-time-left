 ...
 *
 * **왜 묶음인가.** 다섯 문단이 한꺼번에 깔리니 읽기 전에 밀린다는 말을 들었다
 * ("와다다 나오니깐 거부감이 들더라고"). 편지를 통째로 들이미는 대신 스크롤을
 * 내리는 만큼만 펴 보인다. 묶음은 이야기의 마디를 그대로 따른다 —
 * 그 상황이 있고(1), 앱이 하는 일이 있고(2), 걷고 나면 남는 것이 있다(3).
 * 마디가 아니라 개수로 자르면 문단 사이에서 말이 끊긴다.
 */
export function postscriptGroups(): string[][] {
  const promised = promisedMinutes(ARRIVE_EARLY_SEC);
  return [
    // 그 상황. 읽는 사람이 겪어 본 장면 하나로 시작한다.
    ['약속 장소 근처에 일찍 와 버린 날이 있죠.\n카페에 들어가기엔 애매하고, 그냥 서 있기엔 긴 시간.'],
    // 앱이 하는 일. 길을 고르는 것과 걷는 동안의 일은 한 호흡이다.
    [
      `그 시간만큼 걷는 길을 찾아드려요.\n가장 빠른 길 말고, 약속 ${promised}분 전에는 닿는 길로. 오늘 기분에 맞춰서요.`,
      '걷는 동안엔 남은 거리와 닿을 시각만 조용히 알려드려요.\n길에서 벗어나면 한 번 톡, 두드리고요.',
    ],
    // 걷고 나면 남는 것. 도착과 비 오는 날은 둘 다 '그 뒤'의 이야기다.
    [
